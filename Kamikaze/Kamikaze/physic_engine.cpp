...
void Scene::checkAllCollisions(){
	...
	for (Bullet &b : ships[0].bullets) if (b.alive){
		if (collides(b, ships[1])) { ships[1].die(); ships[1].sound.playSound(ShipSound::DIE); }
	}

	for (Bullet &b : ships[1].bullets) if (b.alive) {
		if (collides(b, ships[0])) { ships[0].die(); ships[1].sound.playSound(ShipSound::DIE); }
	}
}
